// esg-system/routes/governanca.js
...
// Cria um novo registro de governança
  ...
  try {
    ...
    if (id) {
      result = await db.query(
        `UPDATE governanca SET categoria = $1, pergunta = $2, avaliacao = $3, oquefazer = $4
      ...
      if (result.rows.length === 0) {
        ...
        result = await db.query(
          `INSERT INTO governanca (categoria, pergunta, avaliacao, oquefazer, empresa_id)
           VALUES ($1, $2, $3, $4, $5)
           RETURNING *`,
...
// Obtém todos os registros de governança da empresa do usuário, incluindo detalhes específicos da empresa
...
router.get('/general', async (req, res) => {
  try {
    const result = await db.query('SELECT id, categoria, pergunta FROM governanca');
    res.json(result.rows);
  } catch (error) {
...
// Atualiza um registro específico de governança, garantindo que pertence à empresa do usuário
router.put('/:id', auth, async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length > 0) {
      ...
      res.status(404).send(`Governança com ID ${id} e empresa_id ${empresa_id} não encontrada.`);
    }
  } catch (error) {
...
// Rota para deletar todos os registros de governança de uma empresa específica
router.delete('/all/:empresaId', auth, async (req, res) => {
  ...
  try {
      const result = await db.query(
          'DELETE FROM governanca WHERE empresa_id = $1',
          [empresaId]
      );
      if (result.rowCount > 0) {
          res.status(200).json({ message: 'Todos os dados de governança foram deletados.' });
      } else {
          res.status(404).json({ message: 'Nenhum dado de governança encontrado para deletar.' });
      }
  } catch (error) {
      console.error("Erro ao deletar todos os dados de governança:", error);
      res.status(500).json({ message: 'Erro ao deletar todos os dados de governança', error: error.message });
  }
});
